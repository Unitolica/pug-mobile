...
>(({ className, children }, ref) => {
  return (
    <CommandList
      ref={ref}
      className={cn(
        "p-2 flex flex-col gap-2 rounded-md scrollbar-thin scrollbar-track-transparent transition-colors scrollbar-thumb-muted-foreground dark:scrollbar-thumb-muted scrollbar-thumb-rounded-lg w-full absolute bg-background shadow-md z-10 border border-muted top-0 bg-zinc-50 ",
        className,
      )}
    >
      {children}
  ...
  return (
    <CommandItem
      ...
      className={cn(
        ...
        className,
        isIncluded && "opacity-100 cursor-default",
        props.disabled && "opacity-100 cursor-not-allowed",
      )}
      onMouseDown={mousePreventDefault}
    >
      {children}
